...
const Navbar1 = () => {
  ...
  return (
    <>
      {/* Fixed navbar at top */}
      <div className="fixed top-0 left-0 right-0 flex justify-center w-full py-6 px-4 z-[99998]">
        <motion.div 
          className={`flex items-center justify-between px-8 py-2.5 rounded-full w-full relative transition-all duration-500 ${
            isDemoMode 
              ? 'max-w-4xl bg-white/95 border border-cyan-400/40 shadow-2xl shadow-cyan-400/30 backdrop-blur-xl' 
              : 'max-w-6xl bg-white/10 border border-white/20 shadow-xl backdrop-blur-md'
          }`}
          animate={{ 
            ...
            <>
              <div className="flex items-center flex-1">
                ...
                <div className="text-sm font-mono">
                  <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-600 via-teal-600 to-cyan-600 font-semibold">
                    {demoState === 'idle' ? 'HeyJarvis Demo Ready' :
                     demoState === 'running' ? 'Demo Running...' :
                     'Demo Complete'}
                  </span>
                </div>
              </div>
              ...
              <div className="flex items-center space-x-2">
                ...
                {(demoState === 'completed' || demoState === 'running') && (
                  <button
                    onClick={resetDemo}
                    className="px-3 py-1 text-xs font-medium bg-gray-600 hover:bg-gray-700 text-white rounded-full transition-colors"
                  >
                    Reset
                  </button>
                )}
